<template>
  <div class="one">
    ...
    <el-button type="primary" @click="add()"
      ><el-icon><Plus /></el-icon>&nbsp;录入</el-button
    >
    <el-table
      :data="tableData"
      style="width: 100%"
    ...
    >
      ...
      <el-table-column prop="title" label="实训主题" width="400">
      </el-table-column>
      <el-table-column prop="abc" label="教学工作实习成绩评定" width="200">
      </el-table-column>
      <el-table-column prop="def" label="班主任工作实习成绩评定" width="200">
      </el-table-column>
      <el-table-column prop="abcdef" label="总成绩" width="200">
      </el-table-column>
...
function data() {
  axios({
    url: 'http://localhost:8082/allachievement',
    method: 'get'
  }).then(res => {
...
function sub() {
  if (from.title === '' || from.a === '' || from.b === '' || from.d === '') {
    ...
    if (isadd.value) {
      const teacher = window.sessionStorage.getItem('username')
      axios({
...
function exportExcel() {
  axios({
    url: 'http://localhost:8888/achievement/export?teacher=' + window.sessionStorage.getItem('username'),
    method: 'get',
    responseType: 'blob',
  }).then(res => {
    // 创建一个blob URL指向下载的内容  
    ...
    link.href = url;
    link.setAttribute('download', '成绩单.xlsx'); // 设置下载文件名  
