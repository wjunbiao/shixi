...
function data() {
  axios({
    url: 'http://localhost:8082/courses?state=1&teacher=' + window.sessionStorage.getItem('username'),
    method: 'get'
  }).then(res => {
...
function query(search) {
  axios({
    url: 'http://localhost:8082/courses2?state=1&search=' +search,
    method: 'get'
  }).then(res => {
...
function sub() {
  if (from.title === '' || from.text === '' || from.time === '') {
    ...
    if (isadd.value) {
      axios({
        url: 'http://localhost:8082/addcourse',
        method: 'get',
        params: {
      ...
      axios({
        url: 'http://localhost:8082/eduitcourse/' + id.value,
        method: 'get',
        params: {
          title: from.title,
          text: from.text,
...
function handleDel(row) {
  axios({
    url: 'http://localhost:8082/delcourse/' + row.id,
    method: 'get'
  }).then(res => {
    data()
    ElMessage({
