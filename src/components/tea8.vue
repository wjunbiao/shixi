    ...
    <div class="one">
        ...
        <el-table
            ...
            <template  #default="scope">
                <el-button @click="handleEdit(scope.row)" type="text" size="small" :disabled="scope.row.teacher!=teacher">编辑</el-button>
                <el-button @click="handleDel(scope.row)" type="text" size="small" :disabled="scope.row.teacher!=teacher">删除</el-button>
            </template>
            </el-table-column>
        </el-table>
        <el-dialog
...
function data() {
  axios({
    url: 'http://localhost:8082/tea_activities',
    method: 'get'
  }).then(res => {
    tableData.value = res.data
    teacher.value = window.sessionStorage.getItem('username')
  })
}

function query(search) {
  axios({
    url: 'http://localhost:8082/tea_activities?search='+search,
    method: 'get'
  }).then(res => {
...
function sub() {
  if (from.title === '' || from.text === '' || from.time === '') {
    ...
    if (isadd.value) {
      axios({
        url: 'http://localhost:8082/addtea_activities',
        method: 'get',
        params: {
      ...
      axios({
        url: 'http://localhost:8082/eduittea_activities/' + id.value,
        method: 'get',
        params: {
          title: from.title,
          text: from.text,
...
function handleDel(row) {
  axios({
    url: 'http://localhost:8082/deltea_activities/' + row.id,
    method: 'get'
  }).then(res => {
    data()
    ElMessage({
