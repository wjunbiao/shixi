...
function data() {
  axios({
    url: 'http://localhost:8082/tea_plan?state=1&teacher=' + window.sessionStorage.getItem('username'),
    method: 'get'
  }).then(res => {
...
function query(search) {
  axios({
    url: 'http://localhost:8082/tea_plan2?state=1&search=' +search,
    method: 'get'
  }).then(res => {
...
function sub() {
  if (from.title === '' || from.text === '') {
    ...
    if (isadd.value) {
      const date = new Date()
      axios({
        url: 'http://localhost:8082/addtea_plan',
        method: 'get',
        params: {
      ...
      axios({
        url: 'http://localhost:8082/eduittea_plan/' + id.value,
        method: 'get',
        params: {
          title: from.title,
          text: from.text
...
function handleDel(row) {
  axios({
    url: 'http://localhost:8082/deltea_plan/' + row.id,
    method: 'get'
  }).then(res => {
    data()
    ElMessage({
